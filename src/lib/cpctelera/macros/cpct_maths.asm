...
;; (end code)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
macro add_REGPAIR_a _rh, _rl
   ;; First Perform RH = E + A
   add _rl    ;; [1] A' = RL + A 
   ld  _rl, a ;; [1] RL' = A' = RL + A. It might generate Carry that must be added to RH
   
   ;; Then Perform RH = RH + Carry 
   adc _rh    ;; [1] A'' = A' + RH + Carry = RL + A + RH + Carry
   sub _rl    ;; [1] Remove RL'. A''' = A'' - RL' = RL + A + RH + Carry - (RL + A) = RH + Carry
   ld  _rh, a ;; [1] Save into RH (RH' = A''' = RH + Carry)
endm
...
macro sub_REGPAIR_a _rh, _rl
   ...
   dec  _rh          ;; [1] --RH (A > RL, so RH must be decremented)
!jmplbl:   
   ...
   ld   _rl, a    ;; [1] Save into RL (RL' = RL - A)
endm
