...
macro cpctm_lutget8 _Table, _TR1, _TR2
    ld   _TR1_TR2, _Table  ;; [3] TR1_TR2 points to the LUT
    
    ;; Compute TR1'TR2 += A
    add  _TR2              ;; [1] | TR2 += A
    ld   _TR2, a           ;; [1] |
    sub  a                 ;; [1] A = 0 (preserving Carry Flag)
    adc  _TR1              ;; [1] | TR1 += Carry
    ld   _TR1, a           ;; [1] |

    ;; A = *(TR1_TR2 + A)
    ld   a, (_TR1_TR2)     ;; [2] A = Value stored at given index from the LUT 
endm
