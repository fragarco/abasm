...
;;
;; Assembly call:
;;    > call cpct_getKeypressedAsASCII
;;
;; Return value:
...
cpct_getKeypressedAsASCII:
   ld  de, cpct_keyboardStatusBuffer ;; [3] DE Points to the start of the keyboard status buffer
   ld   b, 10                        ;; [2] B=10 (Counter of Keyboard Lines to be checked)

kascii_keylines_loop:
   ...
   jr    nz, kascii_key_pressed ;; [2/3] So, if (++A != 0), at least 1 key is pressed in this Keyboard Line
   inc   de                ;; [2] DE++ (Next Keyboard Line)
   djnz  kascii_keylines_loop ;; [3/4] If (--B != 0) Check next Keyboard Line

kascii_no_key_is_pressed:
...
kascii_key_pressed:
   ...
   ld    hl, cpct_keyID_to_ASCII_table ;; [3] HL Points to the key_to_ASCII_table
...
kascii_count_keys_in_line:
   ...
   jr     c, kascii_count_keys_in_line  ;; [2/3] If C=1, key is not pressed, continue checking next key and adding 1 to the count of not pressed
