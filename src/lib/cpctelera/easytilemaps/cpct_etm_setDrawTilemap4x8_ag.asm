...
cpct_etm_setDrawTileMap4x8_ag:
   ...
   ld (dtm48ag_tilesetPtr), hl     ;; [5] Save HL into tilesetPtr placeholder
   ...
   ld (dtm48ag_updateWidth), de   ;; [6] set the difference in updateWidth placeholder
   ...
   ld (dtm48ag_incrementHL),a ;; [4] Set HL increment in its placeholder
   ...
   jp    pe, sdtm48_int_enabled   ;; [3] If interrupts are enabled, EI is the appropriate instruction
   ld     a, &F3        ;; [2] Otherwise, it is DI, so A = Opcode for Disable Interrupts instruction (DI = 0xF3)
sdtm48_int_enabled:
   ld (dtm48ag_restoreI),a ;; [4] Set the Restore Interrupt status at the end with corresponding DI or EI
