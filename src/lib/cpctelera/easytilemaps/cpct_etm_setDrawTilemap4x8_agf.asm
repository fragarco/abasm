...
;;
cpct_setDrawTilemap4x8_agf:
   ;; Set (tilesetPtr) placeholder
   ld (dtm48agf_tilesetPtr), hl     ;; [5] Save HL into tilesetPtr placeholder
   ...
   ld (dtm48agf_widthHeightSet),bc ;; [6]
   ld     a, c                     ;; [1]
   ld (dtm48agf_restoreWidth),a    ;; [4] Set restore width after each loop placeholder
   ...
   ld (dtm48agf_updateWidth), de  ;; [6] set the difference in updateWidth placeholder
   ...
   add    a                    ;; [1] A = 4*Width
   cpl                         ;; [1] A = -4*Width - 1
   add   &50 + 1               ;; [2] A = -4*Width-1 + 0x50+1 = 0x50 - 4*Width
   ld (dtm48agf_incrementHL),a ;; [4] Set HL increment in its placeholder
   ...
   ld     a, &FB            ;; [2] A = Opcode for Enable Interrupts instruction (EI = 0xFB)
   jp    pe, dtm48agf_int_enabled   ;; [3] If interrupts are enabled, EI is the appropriate instruction
     ld   a, &F3            ;; [2] Otherwise, it is DI, so A = Opcode for Disable Interrupts instruction (DI = 0xF3)
dtm48agf_int_enabled:
   ld (dtm48agf_restoreI),a ;; [4] Set the Restore Interrupt status at the end with corresponding DI or EI
