...
;;    > call cpct_etm_drawTilemap4x8_agf
;;
...
;;    MOV = ( inc | dec )
;;
macro dtm48agf_drawSpriteRow MOV
...
dtm48agf_rep:
   ...
   jr     z, dtm48agf_endwidth   ;; [2/3] if (Width==0) jump to endwidth (as no more tiles to be drawn in this row)
   inc   hl             ;; [2]   ++HL (Point to next tile in this tilemap)
   cp   (hl)            ;; [2]   Is A==(HL)? (A:present tile-ID, (HL): next tile-ID in the tilemap)
   jr     z, dtm48agf_rep        ;; [2/3] if (A==(HL)), next tile is equal to present to be drawn, so
...
dtm48agf_drawTile:
   ...
   dtm48agf_drawSpriteRow inc ;; [17] Copy tile line Left-to-Right [>>]
   set    3, h       ;; [ 2] --000---=>--001--- (Next sprite line: 1)
   dtm48agf_drawSpriteRow dec ;; [17] Copy tile line Right-to-Left [<<]
   ...
   jr    nz, dtm48agf_drawTile   ;; [2/3] if (A!=0) we have to draw the tile more times
   ...
   jp    nz, dtm48agf_nexttile   ;; [3] if (IYL!=0) there are still tiles to draw in this row, 
...
dtm48agf_restoreSP equ $+1
   ld    sp, 0000      ;; [3] Restore SP (#0000 is a placeholder)
dtm48agf_restoreI equ $
   ...
   jr     z, dtm48agf_return     ;; [2/3] if (Height==0) then return
   ;;
