...
;;    call  cpct_etm_drawTileBox2x4 ;; Call the function
;; (end code)
;;
...
;;   worst: 111 us
;;
macro etmdb24_mult_de_a
   ;; Displaced digits multiply
 etmdb24_mul:
   rra               ;; [1]   Move next right bit to the Carry
   jr   nc, etmdb24_skip_add ;; [2/3] If No Carry, this bit does not multiply, so we dont add another DE
   add  hl, de       ;; [3]   If Carry, this bit does multiply, so we add DE to the total 

 etmdb24_skip_add:    ;; For the next iteration, we will add 2*DE if next bit from A is on
   sla   e           ;; [2] | DE = 2*DE 
   rl    d           ;; [2] |
   or    a           ;; [1] Check A while resetting Carry (8-bit digit being displaced for multiplication)
   jr   nz, etmdb24_mul ;; [2/3] If A != 0, multiplication hasn't finished yet, continue
endm   

;; Macros for processing HL += A taking carry into account
;;   5 microSecs, 5 bytes
;; 
macro etmdb24_add__hl_a
...
cpct_etm_drawTileBox2x4:
   ...
   ld (etmdb24_simulated_return+1), hl ;; [5] Save return address for simulated return
   ex   de, hl                         ;; [1] HL = Parameters (H=Height in tiles, L=Width in tiles)

   ld (etmdb24_set_map_width + 1), a ;; [4] Save map_width in its placeholder for later use   
   ld    a, l                        ;; [1] A = width of the box in tiles
   ld (etmdb24_draw_next_row + 1), a ;; [4] Save the width into its placeholder to restore it every loop into B
   ld (etmdb24_tb_row_width + 1), a  ;; [4] Save tilebox_row_width in its placeholder for later calculations
   ...
   etmdb24_add__hl_a   ;; [5] HL += 2x, so finally HL = 0x50 * int(y/2) + 0x2000
   ...
   jr    z, etmdb24_dont_add ;; [2/3] If y % 2 == 0, do nothing (we are no a even line, no need to jump)
   ld    a, &38              ;; [2]   If y % 2 != 0, need to calculate where does map start...
   and   d                   ;; [1] ... so check bits 13, 12 and 11 from DE. 
   jr    z, etmdb24_pixline0 ;; [2/3] ... If the 3 bits are 0, we are a at a pixel line 0, go add 0x2000
   ld   de, &E050            ;; [3] ... else, this is pixel line 4, add 0xE050 (jump to next pixel line 0 from pixel line 4)
   jr   etmdb24_doadd        ;; [3] Go to addition line
etmdb24_pixline0:
   ld   de, &2000            ;; [3] DE=0x2000 for jumping to pixel line 4 from pixel line 0
etmdb24_doadd:
...
etmdb24_tb_row_width:
   sub  00                            ;; [2] A = A - tilebox_row_width (A is offset jump to next row)
   ld  (etmdb24_next_row_jump + 1), a ;; [5] Save offset jump to next row in its placeholder
   ...
   jr   etmdb24_draw_next_row ;; [3] Start by drawing first row

;;
;; Draw the tile-box
;;
etmdb24_drawtiles_height:
...
etmdb24_next_row_jump:     ;;     |      the start of the next one 
   ...
   add  &20        ;; [2] 
   ld    d, a      ;; [1]
   and  &38        ;; [2] Check bits 11,12,13 of D to know if we have jumped to a 
                   ;;     ... new character line (pixel line 0, all three bits = 0)
   jr nz, etmdb24_draw_next_row ;; [2/3] Not 0 => Pixel Line 4 => No need to adjust

   ;; We are jumping to a new pixel line 0, 
   ;; so we have to jump to the next character line (adding 0xC050)
   ld    a, e      ;; [1] DE += 0xC050
   add  &50        ;; [2] ... jump 4 lines and, as we will overflow video memory 
   ld    e, a      ;; [1] ... add 0xC050 to move to next character line
   ld    a, d      ;; [1]  
   adc  &C0        ;; [2]
   ld    d, a      ;; [1] 

etmdb24_draw_next_row:
...
;; (B) numtiles (DE') pvideomem (HL) ptilemap
   call  cpct_etm_drawTileRow2x4 ;; [7 + 103W]

   dec  c                   ;; [1] 1 less tile row to draw
   jr  nz, etmdb24_drawtiles_height ;; [2/3] If there still are some tile rows to draw (C!=0), continue

   ;; Drawing ends
etmdb24_simulated_return:
