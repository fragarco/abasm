...
;; consuming. cpct_akp_setFadeVolume becomes available.
...
cpct_akp_digidrumStatus: db 0

;; Loop times
;;    Read here to know the number of times a song has looped
cpct_akp_songLoopTimes: db 0
...
ENDIF
   ...
   xor  a
   ld   (cpct_akp_digidrumStatus), a     ;Reset the Digidrum flag.
...
PLY_Linker_PT: 
   ...
   ld    a, (cpct_akp_songLoopTimes)
   inc   a
   ld    (cpct_akp_songLoopTimes), a
...
;******************************************
   ...
   dw &7DDD  ; ld  a, ixl          ;Save the Register 7 of the Track 3.
   ex  af, af'
...
;***********************************
   ...
   ex  af, af'
  dw &B5DD  ; or ixl                   ;Mix Reg7 from Track3 with Track2+1.
...
;Register 0
   dw &71ED  ;out(c), 0    ; &F400+Register
   ld   b, e
   dw &71ED  ;out(c), 0    ; &F600
   dec  b
   outi                       ; &F400+value
   exx
   out (c), e                 ; &F680
...
PLY_PS_S_SecondByteNeeded:
   db &DD, &2E, 0b1000 ; ld ixl,%1000  ;By defaut, No Noise, Sound.
   ...
   ld  (PLY_PSGReg6), a
   db &DD, &2E, 0b0000 ; ld ixl,%0000  ;Open Noise Channel.
...
PLY_PS_S_SBN_NoNoise:
   ...
   ;No Sound. Stop here.
   dw &2CDD ; inc ixl     ;Set Sound bit to stop the Sound.
   ret
...
PLY_PS_Hard_NoRetrig:
   ...
   db &DD, &2E, 0b1000 ; ld ixl,%1000     ;Sound is always On here (only Independence mode can switch it off).
...
PLY_PS_I_SoundOn:
   db &DD, &2E, 0b1000 ; ld ixl, %1000   ;Sound is on.
   dw &63DD               ; ld ixh, e       ;Save the original note for the Hardware frequency, 
...
PLY_PS_I_SkipSoftwareFrequencyCalculation:
   ...
   inc hl
   ld  (PLY_PSGReg6), a
   dw &7DDD  ; ld a, ixl                    ;Set the Noise bit.
   res  3, a
   dw &6FDD  ; ld ixl, a
   ret
...
cpct_akp_musicInit:   ;; Entry point for assembly calls using registers for parameter passing
   ...
   xor   a                          ;; A = 0
   ld (cpct_akp_songLoopTimes), a  ;; cpct_akp_songLoopTimes = 0
...
IF PLY_UseSoundEffects
   ...
   ;;
   ;; Assembly call (Input parameters on registers):
   ;;    > call cpct_akp_SFXGetInstrument
   ;;
   ;; Details:
...
cpct_akp_setFadeVolume:     ;; ASM Entry point for setFadeVolume
   PLY_SetFadeValue:
