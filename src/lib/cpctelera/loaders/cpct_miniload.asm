...
miniload_init: 
   ...
   ;; Identify pilot Tone
miniload_tone: 
   call  miniload_full        ;; Read 1 pulse
   jr    nc, miniload_init    ;; Is it too long?
   sub    b
   jr    nc, miniload_init    ;; Is it too long?
   inc    h
   jr    nz, miniload_tone    ;; valid TONE
...
miniload_sync: 
   ...
   jr    nc, miniload_init    ;; Is it too long?
   sub    b
   jr     c, miniload_wait    ;; If it belongs to Tone, continue waiting
   inc    h
   jr    nz, miniload_sync    ;; valid SYNC signal
   jr    miniload_byte
...
miniload_bits: 
   ...
   call  miniload_half        ;; Read half pulse
   call   c, miniload_half    ;; Read another half pulse if first was right
   jr    nc, miniload_exit    ;; On error, exit (L != 0 in this case, that will be the error code)
   sub    b
   rl     l          ;; Insert last bit read into the next byte being read
   jr    nc, miniload_bits    ;; Continue reading bits until the byte is full (Carry will appear)
   ...
   jr    nz, miniload_next    ;; If DE != 0, continue reading next byte
