...
cpct_setBit:
   ...
   ;;
   ld    a, &27   ;; [2] A = 0x27 = 0b00100111 => cpl(0xC6) rotated right 3 bits (0xC6 is part of set 0,(hl) instruction, 0xCBC6)
   srl   h        ;; [2] / HL /= 2
   ...
   jr    nz, sb_set_bit ;; [2/3] If (bit_0_of_C != 0) caller wants to set the bit. Otherwise, wants to reset
sb_reset_bit:
      inc a          ;; [1] Bit 0 of A = 1 ==> Change instruction to reset B, (hl) (reversed)
sb_set_bit:          ;;     A = 0bBBB0010S ==> BBB = bit to set/reset, S = selected set or reset (reversed)
   ...
   ld (setbit_instr), a ;; [4] Modify bit instruction to set/reset our target bit
