...
cpct_set2Bits:
   ...
   and   &03         ;; [2] A = L % 4 (Calculate the group of 2 bytes we will be setting from the target byte:
   ...
   and   &03         ;; [2] A = C % 4 (Ensure value to be set is in the range 0-3)
...
s2b_B_is_1:
   ...
   rrca                ;; [1] <|
   rrca                ;; [1] <|
   ld    c, &CF        ;; [2] Mask for leaving out bits 5 & 4
...
s2b_B_is_2:
   ...
   rlca                ;; [1] <|
   ld    c, &F3        ;; [2] Mask for leaving out bits 2 & 3
   jr s2b_end          ;; [3] Done with preparing mask and rotations, continue to setting bits
s2b_B_is_0:
   ld    c, &3F        ;; [2] Mask for leaving out bits 7 & 6
   rrca                ;; [1] <| Move bits to positions 7 & 6 with 2 right rotations
   rrca                ;; [1] <|
   db  &FA ;jp M, xx   ;; [3] Fake jump to gb_end (JP M, xx will be never done, as S is not set. Value XX is got from 
                       ;; .... next 2 bytes, which are "ld C, &FC". Not jumping leaves us 3 bytes from here, at g2b_end)
s2b_B_is_3:
   ld    c, &FC        ;; [2] Mask for leaving out bits 1 & 0
s2b_end:
