...
cpct_get6Bits:
   ...
   jr     c, g6bs23 ;; [2/3] If Carry, bit was 1, so A contained 2 or 3 (01 or 11, remember bits are in inverse order)
   rrca           ;; [1]   No Carry, So A contains 2 or 4. Rotate again to get next bit and check
   jr    nc, g6b0   ;; [2/3] If Not Carry, second bit was 0, so A contained a 0 (00)
...
g6bs23:
   ...
   jr     c, g6b3 ;; [2/3] If Carry, bit was 1, so A contained 3 (11)
...
g6b2:
   ...
   ld     a, (hl) ;; [2] A=byte 1
   and    &0F     ;; [2] Remove first 4 bits of the byte: they're not from the desired value
   ld     b, a    ;; [1] Save these 4 bits into B
   inc   hl       ;; [2] Point HL to byte 2, which has last 2 bits
   ld     a, (hl) ;; [2] A=byte 2
   and    &C0     ;; [2] Remove last 6 bits, as we only want the first 2
...
g6b3:
   ...
   and    &3F     ;; [2] Remove 2 leftmost bits that are not part of the desired value
   ld     l, a    ;; [1] Move return value to L for returning it
   ret            ;; [3] ret
