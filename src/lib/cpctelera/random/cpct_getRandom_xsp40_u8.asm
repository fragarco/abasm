...
cpct_getRandom_xsp40_u8:
   ...
   ld  hl, cpct_seed3_xsp40_u8  ;; [3] HL points to V value (seed 3)
   dec (hl)                     ;; [3] V += 255 (same as decrementing 1 in 8-bits arithmetic)
  
   ;; Second, load 32-bit main seeds (xyzw)
   cpct_seed1_xsp40_u8 equ $+1
   cpct_seed2_xsp40_u8 equ $+4
   ld  de, &1234                ;; [3] Seed 1: values x, z
   ld  hl, &5678                ;; [3] Seed 2: values y, w

   ;; Update x' and z' seed values
   ld  (cpct_seed1_xsp40_u8),hl ;; [5] x' = y, z' = w
   ...
   ld  (cpct_seed2_xsp40_u8), hl  ;; [5] save y' = z and w' = A in 2nd seed value

   ;; Calculate return value doing a final mixing XOR
   ;; operation against a Weyl sequence created by rotating v value
   cpct_seed3_xsp40_u8 equ $+1
   xor &FD         ;; [2] A = w' ^ v' (as this operand contains updated v' value)
