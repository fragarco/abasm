...
;;    > call cpct_getRandom_glfsr16_u8
;;
...
;;
;; Assembly call (Input parameter on L):
;;    > call cpct_getRandom_glfsr16_u16
;;
...
cpct_randUnif_glfsr16_seed equ $+1
   ld  hl, &ABCD    ;; [3] DE = Machine state (seed)
   ...
   rr   l                   ;; [2] |
   jr  nc, rnd_glfsr16_end  ;; [2/3] Carry hold previous rightmost bit. We do XOR operations only if it was a 1.
...
cpct_randUnif_glfsr16_hightaps equ $+1
   xor  &F7         ;; [2] | HL = HL xor TAPS 
   ld   h, a        ;; [1] |  (Bits to be changed according to Galois LFSR for ...
   ld   a, l        ;; [1] |   maximum sequence: period 2^16)

cpct_randUnif_glfsr16_lowtaps equ $+1
   xor  &FB         ;; [2] |
   ld   l, a        ;; [1] |
