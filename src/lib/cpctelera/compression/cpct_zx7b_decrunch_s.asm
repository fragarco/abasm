...
cpct_zx7b_decrunch_s:
        ld      bc, &8000     ;; [3] marker bit at MSB of B
        ld      a, b          ;; [1] move marker bit to A
dec_copyby: 
        inc     c             ;; [1] to keep BC value after LDD
        ldd                   ;; [5] copy literal byte
dec_mainlo:
        call    dec_getbit     ;; [5+(5/11)] read next bit
        jr      nc, dec_copyby ;; [2/3] next bit indicates either literal or sequence

; determine number of bits used for length (Elias gamma coding)
        push    de            ;; [4] store destination on stack
        ld      d, c          ;; [1] D= 0, assume BC=$0000 here (or $8000)
dec_lenval:
        call    nc, dec_getbit ;; [5+(5/11)] don't call first time (to save bytes)
        rl      c              ;; [2] 
        rl      b              ;; [2] insert bit on BC
        call    dec_getbit     ;; [5+(5/11)] more bits?
        jr      nc, dec_lenval ;; [2/3] repeat, final length on BC

; check escape sequence
        inc     c             ;; [1] detect escape sequence
        jr      z, dec_exitdz ;; [2/3] end of algorithm

; determine offset
        ld      e, (hl)        ;; [2] load offset flag (1 bit) + offset value (7 bits)
        dec     hl             ;; [2]
        sll__e                 ;; [2]
        jr      nc, dec_offend ;; [2/3] if offset flag is set, load 4 extra bits
        ld      d, &10         ;; [2] bit marker to load 4 bits
dec_nexbit:
        call    dec_getbit     ;; [5+(5/11)]
        rl      d              ;; [2] insert next bit into D
        jr      nc, dec_nexbit ;; [2/3] repeat 4 times, until bit marker is out
        inc     d              ;; [1] add 128 to DE
        srl     d              ;; [2] retrieve fourth bit from D
dec_offend:
        rr      e              ;; [2] insert fourth bit into E
        ex      (sp), hl       ;; [6] store source, restore destination
        ex      de, hl         ;; [1] destination from HL to DE
        adc     hl, de         ;; [3] HL = destination + offset + 1
        lddr                   ;; [5xsize (BC)]
dec_exitdz: 
        pop     hl              ;; [3] restore source address (compressed data)
        jr      nc, dec_mainlo  ;; [2/3] end of loop. exit with getbit instead ret (-1 byte)
dec_getbit:
        add     a, a            ;; [1] check next bit
