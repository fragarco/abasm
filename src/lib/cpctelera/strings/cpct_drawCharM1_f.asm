...
dcm1f_bg01:
   ...
   ld   hl, &0F0F            ;; [10] HL = 0F0Fh (0F = RRCA)
   ...
   ;;  Assembler    ; Mach.Code  Cycles
   ;;   AND &F0   ; [ E6F0 ]    [ 7] Only first 4 bits are lightened, as this bits represent the bit 0 of 
   ...
   ;;   RRCA        ; [ 0F   ]    [ 4]
   ;;   AND &F0   ; [ E6F0 ]    [ 7] Set low nibble of A to 0 (as it represents bit 1 for our 4 pixels, 
   ...
   ld   hl, &F0E6              ;; [10] HL = F0E6h (E6 F0 = AND F0h)
   ld (dcm1f_start_b1bg), hl   ;; [16] <<= Insert AND &F0 into code for managing byte 1 and byte 2
   ld (dcm1f_start_b2bg+4), hl ;; [16]
   ld   hl, &0418              ;; [10] HL = 0418h (18 04 = JR $+6)
   ld (dcm1f_start_b1bg+2), hl ;; [16] <<= Insert JR $+6 into code for managing byte 1
   ld    a, &38                ;; [ 7] A = 38h (38 = JR C, xx)
...
dcm1f_bg10:
   ...
   ;;                             .... the 4 first pixels (that has to be 1 for pixels activated)   
   ;;   JR  $+6     ; [ 1804 ]    [12] Jump to the end of the dynamic code section
   ld   hl, &0FE6              ;; [10] HL = 0FE6h (E6 0F = AND 0Fh)
...
dcm1f_bg11:
   ...
   ;;     OR &F0  ; [ F6F0 ] b1 [ 7]  -- This is only for byte 1 (dcm1f_start_b1bg)
   ;;     OR &0F  ; [ F60F ] b2 [ 7]  -- This is only for byte 2 (dcm1f_start_b2bg)
   ;;   XOR C       ; [ A9   ]    [ 4] ;; This last piece of code is static, as it never changes, so no need to insert it
   ld (dcm1f_start_b2bg), hl   ;; [16] <<= Insert RRCA;RRCA;RRCA;RRCA into code for managing byte 2
   ld (dcm1f_start_b2bg+2), hl ;; [16]
   ld  hl, &F6A9               ;; [10] HL = E6A9h (A9 = XOR C, F6 = OR xx (value in next byte))
   ld (dcm1f_start_b2bg+4), hl ;; [16] <<= Insert XOR C; OR xx into code for managing bytes 1 and 2
   ld (dcm1f_start_b1bg+4), hl ;; [16]
   ld   a, &0F                 ;; [ 7] A = 0Fh (0F = Value for previous OR in second byte)
...
dcm1f_fg01:
   ...
   ld   hl, &F0E6              ;; [10] HL = F0E6h (E6 F0 = AND F0h)
   ld (dcm1f_start_b1fg), hl   ;; [16] <<= Insert AND &F0 into code for managing byte 1 and byte 2
...
dcm1f_fg10:
   ...
   ;;  Assembler    ; Mach.Code  Cycles
   ;;   AND &F0   ; [ E6F0 ]    [ 7] Only first 4 bits are lightened, as this bits represent the bit 0 of the 4 
...
dcm1f_fg11:
   ...
   ;;   XOR C       ; [ A9   ]    [ 4] Mix A and C to get the same values at the two nibbles of A
   ;;    AND &0F  ; [ E60F ] b1 [ 7]  -- This is only for byte 1 (dcm1f_start_b1fg)
   ;;    AND &F0  ; [ E6F0 ] b2 [ 7]  -- This is only for byte 2 (dcm1f_start_b2fg)
   ;;   XOR C       ; [ A9   ]    [ 4] ;; This last piece of code is static, as it never changes, so no need to insert it
   ld (dcm1f_start_b2fg), hl   ;; [16] <<= Insert RRCA;RRCA;RRCA;RRCA into code for managing byte 2
   ld (dcm1f_start_b2fg+2), hl ;; [16]
   ld   hl, &E6A9              ;; [10] HL = E6A9h (A9 = XOR C, E6 = AND xx (value in next byte))
   ld (dcm1f_start_b2fg+4), hl ;; [16] <<= Insert XOR C; OR xx into code for managing bytes 1 and 2
   ld (dcm1f_start_b1fg+4), hl ;; [16]
   ld    a, &F0                ;; [ 7] A = F0h (F0 = Value for previous AND in second byte)
...
dcm1f_asciiHL:
   ...
   ld    h, &07                ;; [ 7] H = 0x07, because 0x07 * 8 = 0x38, (high byte of 0x3800)
...
dcm1f_start_b1fg:
   defs 7                      ;; [xx] This 7 bytes are filled up with dynamic code for calculating 
...
dcm1f_end_b2bg:
   ...
   and   &07                   ;; [ 7] ... then L % 8 == 0, as it is 8-byte-aligned. 
   ...
   ADD   &08                   ;; [ 7]    so we add it to DE (just by adding 0x08 to D)
   ld    d, a                  ;; [ 4]
   and   &38                   ;; [ 7] We check if we have crossed memory boundary (every 8 pixel lines)
...
dcm1f_8bit_boundary_crossed:
   ...
   ADD   &50                   ;; [ 7]   -- Relocate DE pointer to the start of the next pixel line 
