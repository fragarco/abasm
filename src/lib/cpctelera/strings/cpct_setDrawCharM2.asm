...
;;
;; Details:
;;    This function sets the internal values of <cpct_drawCharM2_inner>, so that next calls
...
;; and inserted into <cpct_drawCharM2_inner>, in the address named <cpct_charm2imc> (drawCharM2 
...
;;
;; Required memory:
;;    ASM bindings  - 30 bytes (+38 from <cpct_drawCharM2_inner> = 68 bytes)
;;      C bindings  - 32 bytes (+38 from <cpct_drawCharM2_inner> = 70 bytes)
;;
;; Time Measures:
;; (start code)
...
cpct_setDrawCharM2:
   ...
   and   &07            ;; [2] Remove upper bits of A to ensure parameter errors do not make the program crash

   ld    hl, sdchm2_mc_table ;; [3] HL Points to the start of the Machine Code Table
   ...
   ;; Copy Selected Machine Code to cpct_drawCharM2_inner
...
sdchm2_mc_table:
   db &3E, &00  ;; Option 00: BG/BG >> LD A, 00
   db &1A, &2F  ;; Option 01: BG/FG >> LD A, (DE) : CPL
   db &1A, &00  ;; Option 10: FG/BG >> LD A, (DE) : NOP
   db &3E, &FF  ;; Option 11: FG/FG >> LD A, FF
