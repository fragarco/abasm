...
cpct_memset_f64_i:
   ...
   and  &03          ;; [2] C = C % 4 (Only the last 2 bits of C are valid, as dividing by 64 should
...
msetf64i_restore_sp equ $+1
   ld    sp, 0000   ;; [3] Placeholder for restoring SP value
   ei                ;; [1] Reenable interrupts   
   
   djnz msetf64i_copyloop ;; [4/3] 1 Less chunk. Continue if there still are more chunks (B != 0)
   dec  c            ;; [1] 256 less chunks (b runned up to 0, decrement c by 1)
   jp   p, msetf64i_copyloop ;; [3] Continue 256 chunks more if C >= 0 (positive)
