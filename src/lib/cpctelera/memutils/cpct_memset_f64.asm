...
cpct_memset_f64:
   ...
   ld   (msetf64_restore_sp), sp ;; [6] Save SP to recover it later on
   ...
   jp   p, msetf64_startcopy ;; [3]  IF C = 0, then
...
msetf64_copyloop:
   ...
   push de           ;; [4]
   push de           ;; [4]
   djnz msetf64_copyloop ;; [4/3] 1 Less chunk. Continue if there still are more chunks (B != 0)
   dec  c            ;; [1] 256 less chunks (b runned up to 0, decrement c by 1)
   jp   p, msetf64_copyloop ;; [3] Continue 256 chunks more if C >= 0 (positive)

msetf64_restore_sp equ $+1
