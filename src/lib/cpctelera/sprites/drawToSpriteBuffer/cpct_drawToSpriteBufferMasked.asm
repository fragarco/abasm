...
;;    > call cpct_drawToSpriteBufferMasked
;;
...
cpct_drawToSpriteBufferMasked:
   ...
   ld (dtspbm_offset_to_next_line), a   ;; [4] Modify the offset size inside the copy loop
   ...
   ld (dtspbm_sprite_width_restore), a  ;; [4] Modify the Sprite Width value inside the copy loop

   ;; Perform the copy
   dtspbm_copy_loop:
      dtspbm_sprite_width_restore equ $+1
      ...
      jr nz, dtspbm_line_loop  ;; [2/3] Repeat line_loop if C!=0 (Iterations pending)
      ...
      dtspbm_offset_to_next_line equ $+1
      ld  a, 00      ;; [2] A = Offset_to_next_line (00 is a place holder that will be self-modified)
      add_de_a       ;; [5] DE += A (Macro)

   djnz dtspbm_copy_loop ;; [3/4] B--, One less iteration to complete Sprite Height
