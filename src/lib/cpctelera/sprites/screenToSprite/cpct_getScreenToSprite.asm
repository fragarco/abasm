...
cpct_getScreenToSprite:
   ...
   ld    (s2sp_restore_width), a ;; [4] Modify sprite with in the code for the loop

   sub   c                    ;; [1] | A = c - c - c = -c 
   sub   c                    ;; [1] |   A contains negative width
   ld    (s2sp_neg_width), a  ;; [4] Modify negative sprite width in the calculation code

   ld    a, b                 ;; [1] A = sprite height

s2sp_next_line:
s2sp_restore_width equ $+1
   ld   bc, &0000    ;; [3] 0000 is a placeholder for the width of a sprite line
...
s2sp_neg_width equ $+1
   ld    bc, &0700   ;; [3] 0700 will be modified to be 07xx, with xx being negative width of the sprite
   ...
   and   &38            ;; [2] We get the 3 bits (12,13 and 14) to test if we have crossed 8-line character boundary
   ld     a, b          ;; [1] Restore A value (previously saved into B)
   jr    nz, s2sp_next_line ;; [2/3]  If bits 12,13 and 14 are 0, we are at the first line of a character 
...
s2sp__8line_character_boundary_crossed:
   ...
   ld    bc, &C050    ;; [3] Value to be added to make HL point to next pixel line in video memory
   add   hl, bc       ;; [3] HL += 0xC050
   jr  s2sp_next_line ;; [3] Continue with next pixel line
