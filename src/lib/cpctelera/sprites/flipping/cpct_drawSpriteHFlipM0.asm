...
cpct_drawSpriteHFlipM0:
   ...
   ld     a, c          ;; [1] A = Width
   ld (dsphfm0_widthRestore), a ;; [4] Set up Width restoration value in C register for each new sprite row
   ld (dsphfm0_addWidth), a     ;; [4] Set up Width to be added to HL for next video memory row to be filled up
   ...
   jr  dsphfm0_firstbyte        ;; [3] First byte does not require C to be restored nor DE/HL to be changed
...
dsphfm0_addWidth equ $+1
   ...
   jr    nz, dsphfm0_nextbyte   ;; [2/3] If A!=0, someone of the 3 bits is not 0, so boundaries have not been crossed
...
dsphfm0_widthRestore equ $+1
   ld     c, 00         ;; [2] Restore Width into C before looping over next row bytes one by one
   jr    dsphfm0_nextbyte       ;; [3] Continue with next sprite row
dsphfm0_return:
