...
cpct_drawSpriteVFlip_f:
   ...
   sub   c                 ;; [1]
   ld  (dspvff_jrOffset), a    ;; [4] Modify JR offset to create the jump we need

   ld    a, b              ;; [1] A = Height (used as counter for the number of lines we have to copy)

dspvff_drawSpriteWidth_next:
   ;; NEXT LINE
...
dspvff_jrOffset equ $+1   ;;
   ...
   jp   nz, dspvff_drawSpriteWidth_next ;; [3] If any bit from {13,12,11} is not 0, we are still inside 
   ...
   ld   bc, &3FB0             ;; [3] We advance destination pointer to next line
   add  hl, bc                ;; [3]  HL -= 0xC050 (+=3FB0)
   jp dspvff_drawSpriteWidth_next ;; [3] Continue copying
