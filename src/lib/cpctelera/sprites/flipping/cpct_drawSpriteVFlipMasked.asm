...
;;    > call cpct_drawSpriteVFlipMasked
;;
...
cpct_drawSpriteVFlipMasked:
   ;; Modify code using width inside drawing loops
   ld (dspvfm_widthRestore), a ;; [4] Save sprite width into its restoration placeholder for row loop
   neg                  ;; [2] A = -A (0 - Sprite Width)
   ld (dspvfm_minusWidthOp), a ;; [4] Save negative sprite width into placeholder for next row calculation

dspvfm_nextSpriteRow:
   ex   de, hl       ;; [1] Switch DE <=> HL to have HL pointing to the sprite and DE to video memory
dspvfm_widthRestore equ $+1
...
dspvfm_nextByte:
   ...
   djnz  dspvfm_nextByte ;; [3/4] B--; While not 0, we are still painting this sprite row, so continue with next Byte

   dec__ixh          ;; [2] IXH--; 1 more row of the sprite completed
   jr     z, dspvfm_return ;; [2/3] If that was the last row, we safely return
...
dspvfm_minusWidthOp equ $+1
   ld    bc, &F700   ;; [3] BC = -(0x800 + sprite width) => Offset to jump to previous row (00 is a placeholder)
   ...
   jr   nz, dspvfm_nextSpriteRow ;; [2/3] If any bit from {13,12,11} is not 0, we are still inside 
   ...
   ld    bc, &3FB0    ;; [3] We advance destination pointer to next row
   add   hl, bc       ;; [3]  HL -= 0xC050 (+=3FB0)
   jr    dspvfm_nextSpriteRow  ;; [3] Continue copying
