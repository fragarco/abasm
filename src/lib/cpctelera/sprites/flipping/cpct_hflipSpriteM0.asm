...
;;          cpct_hflipSpriteM0(4, 8, characterSprite);
...
;;
hfspm0_ld_b_hl     equ &46
hfspm0_jr_one      equ &18 + &100 * (          hfspm0_one  - (hfspm0_varjump + 3))
hfspm0_jr_byteloop equ &18 + &100 * ((hfspm0_nextbyte + 1) - (hfspm0_varjump + 2))

;;
;; SET UP CODE: Set up loop internal "variable jump" (varjump) so that it takes
...
cpct_hflipSpriteM0:
   ...
   jr  c, hfspm0_odd_width   ;; [2/3] Carry = 1, means Odd sprite width
   ...
   ;;
hfspm0_even_width:
   ld  de, hfspm0_jr_byteloop  ;; [3] DE = Opcode for "JR nextbyte+1" operation, to jump to the byte loop
   ld  (hfspm0_varjump), de    ;; [6] Insert "JR nextbyte+1" at varjump location
   jr hfspm0_nextrow           ;; [3] Start processing the loop for the first 2 bytes
...
hfspm0_odd_width: 
   ld   a, hfspm0_ld_b_hl    ;; [2] A = Opcode for "LD B, (HL)" operation
   ld (hfspm0_varjump), a    ;; [4] Insert "LD B, (HL)" at varjump location
   ld  de, hfspm0_jr_one     ;; [3] DE = Opcode for "JR one" operation
   ld (hfspm0_varjump+1), de ;; [6] Insert "JR one" at varjump+1 location (right after LD B, (HL))
...
hfspm0_varjump:
   ...
   jr   hfspm0_one ;; [3] Jump to the second part of the internal loop, to reverse only the central byte
...
hfspm0_one:
   ...
   jr nz, hfspm0_nextbyte ;; [2/3] If C != 0, there are still bytes to be flipped and switched, so continue
   ...
   djnz hfspm0_nextrow ;; [3/4] B-- (Height--). If B!=0, there are still more rows pending, so continue
