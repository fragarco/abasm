...
cpct_drawSpriteVFlip:
   ;; Modify code using width to jump in drawSpriteWidth
   ld (dspvf_widthRestore), a ;; [4] Save sprite width into its restoration placeholder for row loop
   neg                  ;; [2] A = -A (0 - Sprite Width)
   ld (dspvf_minusWidthOp), a ;; [4] Save negative sprite width into placeholder for next row calculation
   ld     a, b          ;; [1] A = height

   ;; Draw the sprite row by row
dspvf_nextSpriteRow:
   ex   de, hl       ;; [1] Put destination into DE and source into HL for copying with LDIR
dspvf_widthRestore equ $+1
   ld   bc, &0000    ;; [3] B=0, C=Sprite Width. This value is a placeholder that gets modified 
...
dspvf_minusWidthOp equ $+1
   ...
   jr   nz, dspvf_nextSpriteRow ;; [2/3] If any bit from {13,12,11} is not 0, we are still inside 
   ...
   jr dspvf_nextSpriteRow  ;; [3] Continue copying
