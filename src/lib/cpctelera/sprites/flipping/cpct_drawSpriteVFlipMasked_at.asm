...
;;    > call cpct_drawSpriteVFlipMasked_at
;;
...
cpct_drawSpriteVFlipMasked_at:
   ;; Modify code using width inside drawing loops
   ld__a_ixl            ;; [2] A = width
   ld (dspvfmat_widthRestore), a ;; [4] Save sprite width into its restoration placeholder for row loop
   neg                  ;; [2] A = -A (0 - Sprite Width)
   ld (dspvfmat_minusWidthOp), a ;; [4] Save negative sprite width into placeholder for next row calculation
...
dspvfmat_nextByte:
   ...
   jr   nz, dspvfmat_nextByte ;; [2/3] While not 0, we are still painting this sprite line 

   dec__ixh          ;; [2] IXH--; 1 more row of the sprite completed
   jr    z, dspvfmat_return   ;; [2/3] If that was the last row, we safely return

dspvfmat_widthRestore equ $+2
...
dspvfmat_minusWidthOp equ $+1
   add   &00         ;; [2] 00 is a placeholder for -width value
   ld     e, a       ;; [1] |
   ld     a, d       ;; [1] |
   adc   &F7         ;; [2] | We substract 0x800 plus the width of the sprite 
   ld     d, a       ;; [1] | 
   ...
   xor   &38         ;; | [2] ... inverted (so that they 3 turn to 0 if they were 1)
   jr   nz, dspvfmat_nextByte ;; [2/3] If any bit from {13,12,11} is not 0, we are still inside 
   ...
   add   &B0       ;; [2] | Relocate DE pointer to the start of the next pixel line:
   ...
   adc   &3F       ;; [2] | movement then jumping to previous pixel line, only 50 bytes backwards
   ld    d, a      ;; [1] | Calculations are made with 8 bit maths as it is faster than other alternatives here

   jr  dspvfmat_nextByte ;; [3] Jump to continue with next pixel line
