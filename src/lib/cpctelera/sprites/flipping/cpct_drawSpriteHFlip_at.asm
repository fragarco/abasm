...
cpct_drawSpriteHFlip_at:
   ;; Set Up placeholder for flipping table
   ld (dsphf_tablePage), a    ;; [4] Set Up Most Significant Byte of Flipping table to be restored before drawing each new row

   ;; Set Up placeholders to restore sprite width for each new row
   ld__a_ixl            ;; [2] A = Width
   ld (dsphf_widthRestore), a ;; [4] Set up Width restoration value in C register for each new sprite row
   ld (dsphf_addWidth), a     ;; [4] Set up Width to be added to HL for next video memory row to be filled up
   ...
   jr  dsphf_firstbyte  ;; [3] First byte does not require C to be restored nor DE/HL to be changed

dsphf_nextrow:
   ex    de, hl         ;; [1] Interchange again DE<=>HL to have them in their normal configuration
dsphf_tablePage equ $+1 ;; ... HL=Source Sprite, DE=Video Memory location
...
dsphf_firstbyte:
   ...
   jr    nz, dsphf_nextbyte ;; [2/3] If C!=0, there are more bytes, so continue with nextbyte

   ;; Finished drawing present sprite row to screen. Count 1 less row
   dec__ixh               ;; [2]   IXH-- (One less sprite row to go)
   jr     z, dsphf_return ;; [2/3] if IXL==0, all rows are finished, then return

   ;; Set Up everything for Next Sprite Row

dsphf_widthRestore equ $+2
...
dsphf_addWidth equ $+1
   ...
   jr    nz, dsphf_nextrow ;; [2/3] If A!=0, someone of the 3 bits is not 0, so boundaries have not been crossed
   ...
   jr    dsphf_nextrow  ;; [3] Continue with next sprite row
dsphf_return:
