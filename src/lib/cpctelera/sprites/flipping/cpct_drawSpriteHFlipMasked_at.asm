...
;;    > call cpct_drawSpriteHFlipMasked_at
;;
...
cpct_drawSpriteHFlipMasked_at:
   ;; Set Up placeholder for flipping table
   ld (dsphfm_tablePage), a    ;; [4] Set Up Most Significant Byte of Flipping table to be restored before drawing each new row

   ;; Set Up placeholders to restore sprite width for each new row
   ld__a_ixl            ;; [2] A = Width
   ld (dsphfm_widthRestore), a ;; [4] Set up Width restoration value in C register for each new sprite row
   ld (dsphfm_addWidth), a     ;; [4] Set up Width to be added to HL for next video memory row to be filled up
   ...
   jr  dsphfm_firstbyte  ;; [3] First byte does not require C to be restored nor DE/HL to be changed

dsphfm_nextrow:
dsphfm_tablePage equ $+1 ;; ... HL=Source Sprite, DE=Video Memory location
...
dsphfm_firstbyte:
   ...
   jr    nz, dsphfm_nextbyte   ;; [2/3] If C!=0, there are more bytes, so continue with nextbyte

   ;; Finished drawing present sprite row to screen. Count 1 less row
   dec__ixh             ;; [2]   IXH-- (One less sprite row to go)
   jr     z, dsphfm_return     ;; [2/3] if IXL==0, all rows are finished, then return

   ;; Set Up everything for Next Sprite Row

dsphfm_widthRestore equ $+2
...
dsphfm_addWidth equ $+1
   ld    bc, &0800      ;; [3] BC = 0x800 + width  (Offset from now to right-most byte of next row)
   ...
   and   &38            ;; [2]   Isolate bits 13,12,11 
   jr    nz, dsphfm_nextrow ;; [2/3] If A!=0, someone of the 3 bits is not 0, so boundaries have not been crossed
