...
cpct_vflipSprite:
   ...
   ld (vfsp_wrestore), a  ;; [5] Store sprite width in its restore-placeholder
   ...
   jr    vfsp_byte_loop   ;; [3] Jump over next-row-loop for the first iteration
...
vfsp_byte_loop:
   ...
   jr   nz, vfsp_byte_loop   ;; [2/3] If C!=0, continue with next byte of this row

   ;; Finished processing present row. Decrement B (height counter) and
   ;; continue with next row, in case
   djnz vfsp_row_loop        ;; [3/4] --B. If B!=0, continue with next row
