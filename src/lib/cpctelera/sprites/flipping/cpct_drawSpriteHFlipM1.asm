...
cpct_drawSpriteHFlipM1:
   ...
   ld     a, c          ;; [1] A = Width
   ld (dsphfm1_widthRestore), a ;; [4] Set up Width restoration value in C register for each new sprite row
   ld (dsphfm1_addWidth), a     ;; [4] Set up Width to be added to HL for next video memory row to be filled up
   ...
   jr  dsphfm1_firstbyte ;; [3] First byte does not require C to be restored nor DE/HL to be changed
...
dsphfm1_addWidth equ $+1
   ...
   jr    nz, dsphfm1_nextbyte ;; [2/3] If A!=0, someone of the 3 bits is not 0, so boundaries have not been crossed
...
dsphfm1_widthRestore equ $+1
   ld     c, 00         ;; [2] Restore Width into C before looping over next row bytes one by one
   jr    dsphfm1_nextbyte ;; [3] Continue with next sprite row
dsphfm1_return:
