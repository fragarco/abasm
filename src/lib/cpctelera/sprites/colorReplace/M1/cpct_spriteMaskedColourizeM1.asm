...
;;    > call cpct_spriteMaskedColourizeM1
;;
...
spmcm1_loop:
   ...
   ;; Make HL point to next byte in the array and decrement BC 
   ;; HL will be incremented 2 as are interleaved mask bytes.
   inc   hl       ;; [2] ++HL
   inc   hl       ;; [2] ++HL
   dec    c       ;; [1] --C
   jr    nz, spmcm1_loop ;; [2/3] if (C > 0) then BC != 0, continue with the loop
   djnz  spmcm1_loop     ;; [3/4] if (--B > 0) then BC != 0, continue with the loop
