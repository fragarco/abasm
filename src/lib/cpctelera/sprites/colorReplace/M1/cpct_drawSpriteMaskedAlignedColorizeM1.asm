...
;;
;; C Definition:
;;    void <cpct_drawSpriteMaskedAlignedColorizeM1> (void* *sprite*, void* *memory*, <u8> *width*, <u8> *height*, const void* *pmasktable0*, <u16> *rplcPat*) __z88dk_callee;
;;
...
;;    > call cpct_drawSpriteMaskedAlignedColorizeM1
;;
...
cpct_drawSpriteMaskedAlignedColorizeM1
   push  af        ;; [4] Save AF (Source Sprite Pointer) in Stack
   ld   (dspmacm1_aligned_mask_table), ix ;; [6] Set Aligned Mask Table pointer in placeholder
...
dspmacm1_aligned_mask_table equ $+1 
   ...
   jp    nz, dspmacm1_width_loop ;; [2/3] While not 0, we are still painting this sprite line 
   
   pop   de        ;; [3] Restore DE start line (DestMem)   
   dec   b         ;; [1] --B holds sprite height. We decrease it to count another pixel line finished
   jp    z, dspmacm1_end_sprite_colourize ;; [2/3] If 0, we have finished the last sprite line.
   
   ld__c_iyh       ;; [2] C = IYH (Sprite Width)
   ld    a, d      ;; [1] Start of next pixel line normally is 0x0800 bytes away.
   add   &08       ;; [2] so we add it to DE (just by adding 0x08 to D)
   ld    d, a      ;; [1]
   and   &38       ;; [2] We check if we have crossed memory boundary (every 8 pixel lines)
   jp    nz, dspmacm1_height_loop  ;; [2/3] .. by checking the 4 bits that identify present memory line. 

   ld    a, e      ;; [1] If 0, we have crossed boundaries then DE = DE + 0xC050h
   add   &50       ;; [2] -- Relocate DE pointer to the start of the next pixel line:
   ...
   adc   &C0       ;; [2] -- 50 bytes forwards (which is what we want to move it to the next pixel line)
   ld    d, a      ;; [1] -- Calculations are made with 8 bit maths as it is faster than other alternatives here
   jp    dspmacm1_height_loop ;; [3] Jump to continue with next pixel line 

dspmacm1_end_sprite_colourize:
