...
;;    > call cpct_drawSpriteColorizeM0
;;
...
;;       cpct_drawSpriteColorizeM0(g_star, vmem, G_STAR_W, G_STAR_H, replacePatColor);
;;    }
;; (end code)
;;
;; Known limitations:
...
dspcm0_width_loop:
   ...
   ld   (de), a    ;; [2] Copy colored byte in screen
   ...
   jp    nz, dspcm0_width_loop ;; [2/3] While not 0, we are still painting this sprite line 
   
   pop   de        ;; [3] Restore DE start line (DestMem)   
   dec   b         ;; [1] --B holds sprite height. We decrease it to count another pixel line finished
   jp    z, dspcm0_end_sprite_colourize ;; [2/3] If 0, we have finished the last sprite line.
   ...
   jp    nz, dspcm0_height_loop ;; [2/3] .. by checking the 4 bits that identify present memory line. 
   ...
   jp    dspcm0_height_loop ;; [3] Jump to continue with next pixel line 

dspcm0_end_sprite_colourize:
