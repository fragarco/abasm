...
; correspendiendo a pantalla juego y superbuffer
	...
	ld (#tile1deA+1),hl
	ld l,8 (iy)
	ld h,9 (iy) ; inicio datos en l√≠nea pantalla total
	call print_tileA
	ld de,#10
	add iy,de
	pop af
	...
	ld (#paresI),A
	ret
; HL l√≠nea correspondiente a los datos origen del mapa global
; BC tiene el n√∫mero de columna a buscar
	print_tileA:
	ld bc,#0000
	ld de,#_test_map2
	add hl,de
	...
	add hl,hl
	ld de,#_bloques
	add hl,de ; EN HL est√°n los datos del bloque
	paresI:
	NOP ; INC HL = #0x23
	push hl
...
;code.c:358: sprite01.visible=3;
	...
	ld	(hl),#0x03
	ret
;code.c:373: void act_visible(void)              // dependiendo del scroll y de la posiciÛn del sprite, se activa o desactiva
;	---------------------------------
...
;code.c:514: sprite00.frame=0;
	ld	hl,#_sprite00 + 19
	ld	(hl),#0x00
;code.c:515: sprite00.num=1; // num=0 mosca, num=1 ratÛn
	ld	hl,#_sprite00 + 20
	ld	(hl),#0x01
...
;code.c:530: sprite01.frame=0;
	ld	hl,#_sprite01 + 19
	ld	(hl),#0x00
;code.c:531: sprite01.num=1; // num=0 mosca, num=1 ratÛn
	ld	hl,#_sprite01 + 20
	ld	(hl),#0x01
...
;code.c:565: if (col<ANCHO_PANTALLA_SC)
	...
	sbc	a, d
	jr	NC,00109$
;code.c:568: if (nave.cx>=(40+e))  // se comprueba para ver si hay que hacer scroll al cambiar la direcciÛn
	ld	hl, #(_nave + 0x0008) + 0
	ld	d,(hl)
	ld	iy,#_e
	ld	l,0 (iy)
...
;code.c:592: if (col>4 )
	...
	sbc	a, 1 (iy)
	jr	NC,00119$
;code.c:595: if (nave.cx<=(10-f))  // se comprueba para ver si hay que hacer scroll al cambiar la direcciÛn
	ld	hl, #(_nave + 0x0008) + 0
	ld	e,(hl)
	ld	iy,#_f
	ld	h,0 (iy)
