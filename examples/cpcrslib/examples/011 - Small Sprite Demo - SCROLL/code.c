...
unsigned char sc;
unsigned char d;		// para ver si el sprite controlado por el usuario sigue la misma dirección o ha habido cambio
unsigned char e, f;		// para controlar si el scroll cambia de dirección
unsigned char vs1, vs2; // Controlan si habrá scroll total o medio
...
void act_visible(void)              // dependiendo del scroll y de la posición del sprite, se activa o desactiva
{
    char vpointerH;
...
main()
{
    ...
    sprite00.tipo=3;
    sprite00.frame=0;
    sprite00.num=1; // num=0 mosca, num=1 ratón
    sprite00.dir=0;
    ...
    sprite01.tipo=3;
    sprite01.frame=0;
    sprite01.num=1; // num=0 mosca, num=1 ratón
    sprite01.dir=0;
    ...
    while(1)
    {
        ...
        if (cpc_TestKey(0)==1 && nave.cx<=40)   // DERECHA
        {
            ...
            {
                {
                    if (nave.cx>=(40+e))  // se comprueba para ver si hay que hacer scroll al cambiar la dirección
                    {
                        vs2=1;
                        sc=1;				// SCROLL (<-)
                    }
        ...
        if (cpc_TestKey(1)==1 && nave.cx>0)   // IZQUIERDA
        {
            ...
            {
                {
                    if (nave.cx<=(10-f))  // se comprueba para ver si hay que hacer scroll al cambiar la dirección
                    {
                        vs2=2;
                        sc=1;				// SCROLL (->)
                    }
        ...
        if (sc!=0)  	// Tipo de scroll que se realizará (medio=solo se cambia el punto de lectura de pantalla o total=cambio de coordenadas)
        {
